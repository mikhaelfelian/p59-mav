...
class RolePermissionModel extends \App\Models\BaseModel
{
    ...
    {
        $sql = 'SELECT * FROM module_permission mp LEFT JOIN module m ON m.id_module = mp.id_module ORDER BY m.judul_module';
    ...
    public function hasAllPermission($id_role) {
        $sql = '
                SELECT COUNT(*) AS jml FROM module_permission mp 
                LEFT JOIN module m ON m.id_module = mp.id_module
                LEFT JOIN 
                
                ( SELECT * FROM role_module_permission WHERE id_role = ' . $id_role . ' )
                AS tabel ON tabel.id_module_permission = mp.id_module_permission
                WHERE tabel.id_role IS NULL';
    ...
    public function getListDataPermission() {
        ...
        $sql = 'SELECT COUNT(*) AS jml_data FROM module_permission mp
                LEFT JOIN module m ON m.id_module = mp.id_module
                LEFT JOIN 
                ( SELECT * FROM role_module_permission WHERE id_role = ' . $_GET['id'] . ' )
                AS tabel ON tabel.id_module_permission = mp.id_module_permission ' . $where;
        $total_filtered = $this->db->query($sql)->getRowArray()['jml_data'];
        
        // Query Data
        $start = $this->request->getPost('start') ?: 0;
        $length = $this->request->getPost('length') ?: 10;
        $sql = 'SELECT mp.*, m.nama_module, m.judul_module, tabel.id_role FROM module_permission mp 
                LEFT JOIN module m ON m.id_module = mp.id_module
                LEFT JOIN 
                ( SELECT * FROM role_module_permission WHERE id_role = ' . $_GET['id'] . ' )
                AS tabel ON tabel.id_module_permission = mp.id_module_permission
    ...
    public function getListData() {
        ...
        $start = $this->request->getPost('start') ?: 0;
        $length = $this->request->getPost('length') ?: 10;
        $sql = 'SELECT role.*, jml_module, COUNT(mp.id_module_permission) AS jml_permission 
                FROM role r
                LEFT JOIN role_module_permission rmp ON rmp.id_role = r.id_role
                LEFT JOIN module_permission mp ON mp.id_module_permission = rmp.id_module_permission
                LEFT JOIN (
                    SELECT id_role, COUNT(m.id_module) AS jml_module
                    FROM (SELECT r.id_role, mp.id_module
                        FROM role r
                        LEFT JOIN role_module_permission rmp ON rmp.id_role = r.id_role
                        LEFT JOIN module_permission mp ON mp.id_module_permission = rmp.id_module_permission
                        GROUP BY r.id_role, mp.id_module
                    ) AS tabel
                    GROUP BY id_role
                ) AS tabel ON tabel.id_role = r.id_role
                ' . $where . '
                GROUP BY id_role
