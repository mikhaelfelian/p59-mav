...
class PermissionModel extends \App\Models\BaseModel
{
    ...
    */
    ...
    public function getListData($where) {
        ...
        $sql = 'SELECT COUNT(*) AS jml_data FROM module_permission mp LEFT JOIN module m ON m.id_module = mp.id_module ' . $where;
        $total_filtered = $this->db->query($sql)->getRowArray()['jml_data'];
        
        // Query Data
        $start = $this->request->getPost('start') ?: 0;
        $length = $this->request->getPost('length') ?: 10;
        $sql = 'SELECT * FROM module_permission mp LEFT JOIN module m ON m.id_module = mp.id_module
