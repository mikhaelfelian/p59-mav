...
<div class="card">
	...
	<div class="card-body">
		...
		<?php if (isset($role['id_role']) && $role['id_role'] == 4) { ?>
			<!-- Bayar Semua Button -->
		...
		<?php } ?>
		<?php
		// Define columns for DataTables, conditionally show select-all checkbox for role id 4 only
		$column = [];

		if (isset($role['id_role']) && $role['id_role'] == 4) {
			$column['ignore_search_select'] = '<input type="checkbox" class="form-check-input" id="selectAllPaylater" title="Pilih semua di halaman ini">';
		}
		...
		function openBulkPaymentDialog() {
			...
			bootboxDialog = bootbox.dialog({
				...
				buttons: {
					...
					success: {
						label: '<i class="fas fa-check me-1"></i> Bayar',
			...
			$.ajax({
				url: baseURL + 'agent/paylater/pay-mass',
				type: 'POST',
				data: payload,
				dataType: 'html',
				success: function (html) {
					bootboxDialog.find('.modal-body').html(html);
					var formToken = bootboxDialog.find('input[name="' + csrfName + '"]').val();
					if (formToken) {
						csrfHash = formToken;
					}
