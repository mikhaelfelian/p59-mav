...
class Sales extends BaseController
{
    ...
    public function store()
    {
        ...
        // Get sale_channel for invoice number generation (default to offline)
        $saleChannel = $postData['sales_channel'] ?? self::CHANNEL_OFFLINE;
        
        // Generate invoice number automatically (only for new records)
        $invoiceNo = null;
        if (empty($id)) {
            // Generate new invoice number
            $invoiceNo = $this->model->generateInvoiceNo($saleChannel);
            log_message('info', 'Sales::store - Generated invoice number: ' . $invoiceNo);
        } else {
            // For updates, use existing invoice number from database
            $existingSale = $this->model->find($id);
            if ($existingSale) {
                $invoiceNo = $existingSale['invoice_no'] ?? null;
            }
            if (empty($invoiceNo)) {
                // Fallback: generate new if not found
                $invoiceNo = $this->model->generateInvoiceNo($saleChannel);
                log_message('warning', 'Sales::store - Invoice number not found for ID ' . $id . ', generated new: ' . $invoiceNo);
            }
        }
        ...
        if ($platformId) {
            ...
            if ($platform && $gwStatus === '1') {
                ...
                if ($this->salesGatewayLogModel->invoiceExists($invoiceNo)) {
                    log_message('warning', 'Sales::store - Invoice number already sent to gateway: ' . $invoiceNo . '. Generating new invoice number (gateway rule: cannot reuse).');
                    $invoiceNo = $this->model->generateInvoiceNo($saleChannel);
                    log_message('info', 'Sales::store - New invoice number generated: ' . $invoiceNo);
                }
                
                // Step 2: Check if invoice number already exists in database
                $existingSale = $this->model->where('invoice_no', $invoiceNo)->first();
                if ($existingSale) {
                    log_message('warning', 'Sales::store - Invoice number already exists in database: ' . $invoiceNo . '. Generating new invoice number.');
                    $invoiceNo = $this->model->generateInvoiceNo($saleChannel);
                    log_message('info', 'Sales::store - New invoice number generated: ' . $invoiceNo);
                }
                
                // Step 3: Check gateway for existing payment (optional - for reusing existing payments)
                log_message('info', 'Sales::store - Checking gateway for existing payment with orderId: ' . $invoiceNo);
                ...
                if ($gatewayStatusResponse !== null) {
                    ...
                    log_message('info', 'Sales::store - Payment exists in gateway for invoice: ' . $invoiceNo . ', reusing existing payment');
                ...
                if ($gatewayResponse === null) {
                    ...
                    while ($retryCount < $maxRetries && $gatewayResponse === null) {
                        ...
                        log_message('info', 'Sales::store - Gateway API Payload (attempt ' . ($retryCount + 1) . '): ' . json_encode($apiData, JSON_PRETTY_PRINT));
                        ...
                        if ($gatewayResponse === null) {
                            ...
                            if ($lastError && stripos($lastError, 'different request payload') !== false) {
                                ...
                                log_message('info', 'Sales::store - Generated new invoice number for retry: ' . $invoiceNo);
                ...
                try {
                    ...
                    if ($latestGatewayResponse !== null) {
                        ...
                        if ($paymentRecord) {
                            ...
                            log_message('info', 'Sales::store - Updated payment response with latest gateway data for invoice: ' . $checkInvoiceNo);
                        }
                    }
                } catch (\Exception $e) {
